...
La fuente consultada para esta clase es 'Video 9: Seguridad y privacidad'. En nuestro recorrido la conectamos con el tema 'Técnicas pre-mortem para prevenir fallos' porque queremos estudiar técnicas pre-mortem para prevenir fallos desde un problema real. La fuente plantea: La seguridad es una parte esencial de la arquitectura, no un tema opcional que se resuelve al final. Un sistema puede ser rápido y funcional, pero si no maneja bien autenticación, autorización, cifrado, validación de datos y control de acceso, puede poner en riesgo información crítica y la confianza del usuario. El video llama la atención sobre el hecho de que los problemas de seguridad no siempre aparecen como fallas técnicas evidentes: muchas veces son decisiones de diseño que dejan brechas invisibles.

También se habla de privacidad como un principio arquitectónico, no solo como cumplimiento. Diseñar con privacidad implica limitar datos, controlar acceso, definir responsabilidades y evitar recopilar más información de la necesaria. La arquitectura debe pensar en la seguridad desde las capas más básicas hasta el comportamiento del sistema en producción. En la plataforma logística, esto aparece cuando durante una alta demanda, el sistema debe detectar latencia, recuperar mensajes fallidos y mantener una experiencia aceptable. Primero entenderemos la fuente y después construiremos la decisión.
...
Te planteo el problema directamente: La seguridad es una parte esencial de la arquitectura, no un tema opcional que se resuelve al final.
...
Aquí quiero que mires el riesgo humano de esta idea. La seguridad debe ser un eje arquitectónico, no un detalle a último momento. Pregúntate quién podría quedar expuesto si la ignoramos y qué control de diseño reduciría ese riesgo en la plataforma logística.

### 2. La confianza del usuario depende del manejo responsable de datos.

Detente en la consecuencia de esta idea: La confianza del usuario depende del manejo responsable de datos. Relaciónala con una decisión concreta del proyecto, señala qué alternativa descartarías y explícame por qué.

### 3. Hay que proteger no solo la aplicación, sino sus flujos de datos y sus dependencias.

Detente en la consecuencia de esta idea: Hay que proteger no solo la aplicación, sino sus flujos de datos y sus dependencias. Relaciónala con una decisión concreta del proyecto, señala qué alternativa descartarías y explícame por qué.

### 4. La privacidad implica principio de mínimo privilegio y minimización de datos.

Aquí quiero que mires el riesgo humano de esta idea. La privacidad implica principio de mínimo privilegio y minimización de datos. Pregúntate quién podría quedar expuesto si la ignoramos y qué control de diseño reduciría ese riesgo en la plataforma logística.

### 5. Los fallos de seguridad suelen ser resultado de decisiones de diseño y no solo de bugs aislados.

Aquí quiero que mires el riesgo humano de esta idea. Los fallos de seguridad suelen ser resultado de decisiones de diseño y no solo de bugs aislados. Pregúntate quién podría quedar expuesto si la ignoramos y qué control de diseño reduciría ese riesgo en la plataforma logística.

### 6. La arquitectura más segura es la que anticipa riesgos antes de que el sistema entre en producción.

Detente en la consecuencia de esta idea: La arquitectura más segura es la que anticipa riesgos antes de que el sistema entre en producción. Relaciónala con una decisión concreta del proyecto, señala qué alternativa descartarías y explícame por qué.
...
## 🔀 Opciones para resolver técnicas pre-mortem para prevenir fallos
