# Video 15: Acoplamiento entre servicios y sistemas
...
Vienes de trabajar apis y contratos de integración. No vamos a repetirlo: lo usaremos como punto de partida para estudiar acoplamiento entre servicios y sistemas y añadir una decisión nueva al expediente.

Lo que construyas aquí será la base para la próxima conversación: infraestructura, despliegue y entorno de ejecución.
...
Cuando el acoplamiento es alto, hacer cambios implica romper varias piezas del sistema. Cuando la cohesión es baja, una entidad se vuelve confusa y difícil de mantener. La calidad estructural del software se mejora cuando se reducen dependencias innecesarias y se ordenan bien las responsabilidades. El equipo te pide una decisión sobre acoplamiento entre servicios y sistemas, pero todavía no existe una respuesta única. Tu primera pista es esta idea de la fuente: El acoplamiento alto genera fragilidad.
...
## 🎬 Apertura: Acoplamiento entre servicios y sistemas
Hoy vamos a trabajar una situación concreta: Este video explica dos conceptos centrales en arquitectura: acoplamiento y cohesión. El acoplamiento mide cuán dependientes son componentes entre sí; la cohesión mide qué tan relacionadas están las responsabilidades dentro de un mismo módulo o servicio. La mejor arquitectura busca baja dependencia entre partes y alta claridad dentro de cada parte.

Cuando el acoplamiento es alto, hacer cambios implica romper varias piezas del sistema. Cuando la cohesión es baja, una entidad se vuelve confusa y difícil de mantener. La calidad estructural del software se mejora cuando se reducen dependencias innecesarias y se ordenan bien las responsabilidades. No quiero que empieces por un diagrama ni por una tecnología. Quiero que me expliques qué problema aparece aquí y por qué merece una decisión arquitectónica propia.

Antes de entrar en acoplamiento entre servicios y sistemas, quiero que escuchemos primero la idea central de la fuente del curso: Este video explica dos conceptos centrales en arquitectura: acoplamiento y cohesión. El acoplamiento mide cuán dependientes son componentes entre sí; la cohesión mide qué tan relacionadas están las responsabilidades dentro de un mismo módulo o servicio. La mejor arquitectura busca baja dependencia entre partes y alta claridad dentro de cada parte.

Cuando el acoplamiento es alto, hacer cambios implica romper varias piezas del sistema. Cuando la cohesión es baja, una entidad se vuelve confusa y difícil de mantener. La calidad estructural del software se mejora cuando se reducen dependencias innecesarias y se ordenan bien las responsabilidades. En arquitectura no aprendemos una palabra para repetirla en un diagrama; aprendemos a reconocer una situación, analizar alternativas y tomar una decisión defendible.

Imagina que estamos frente a una pizarra. Yo te miro y te pregunto: ¿qué está pasando en este caso?, ¿quién depende de que esto funcione?, ¿qué información nos falta? No me respondas todavía con nombres de herramientas. Primero cuéntame qué problema ves en acoplamiento entre servicios y sistemas. Esa primera respuesta me permite saber si estamos entendiendo el tema o si solo estamos repitiendo soluciones conocidas.
...
## 💬 La pregunta que vamos a resolver sobre acoplamiento entre servicios y sistemas
¿Qué problema real resuelve acoplamiento entre servicios y sistemas y cómo demostraríamos que la solución es adecuada?

## 🧠 Entender acoplamiento entre servicios y sistemas desde el caso
### 📚 Lo que la fuente nos enseña sobre acoplamiento entre servicios y sistemas
La fuente describe este tema así: Este video explica dos conceptos centrales en arquitectura: acoplamiento y cohesión. El acoplamiento mide cuán dependientes son componentes entre sí; la cohesión mide qué tan relacionadas están las responsabilidades dentro de un mismo módulo o servicio. La mejor arquitectura busca baja dependencia entre partes y alta claridad dentro de cada parte.
...
Quiero que empecemos por la afirmación que trae la fuente: El acoplamiento alto genera fragilidad. Si la tomamos en serio, acoplamiento entre servicios y sistemas deja de ser una etiqueta y se convierte en una decisión que debemos observar en el sistema.

Ahora conectemos esa afirmación con la siguiente: La cohesión alta mejora claridad y mantenibilidad. Pregúntate qué componente, actor o regla del negocio queda afectado. No me interesa que repitas la frase; me interesa que puedas señalar dónde aparece en el caso logístico.

La tercera conversación es sobre las consecuencias: La arquitectura debe disminuir dependencias innecesarias.. Aquí es donde una propuesta deja de ser teórica. Dime qué ganamos, qué sacrificamos y qué evidencia nos permitiría revisar la elección.

Finalmente, la fuente añade: Un sistema con responsabilidades bien definidas es más fácil de evolucionar. Esta idea nos ayuda a completar el análisis y a evitar una solución parcial. Cuando terminemos, deberás poder relacionar este principio con una decisión concreta del proyecto.
...
## ❓ Preguntas para pensar en acoplamiento entre servicios y sistemas
- **Te pregunto:** ¿Qué tan acoplado está mi sistema en este momento? **La razón:** así conectamos el tema con el problema real en lugar de aplicarlo por moda.
...
## 💡 Lo esencial sobre Acoplamiento entre servicios y sistemas
...
- La calidad estructural se refleja en cómo se organizan los componentes.
- Un buen diseño minimiza efectos colaterales de un cambio.
...
### 🔎 Mi lectura de acoplamiento entre servicios y sistemas como profesor
...
## 🧩 Acoplamiento entre servicios y sistemas: caso de la plataforma logística
...
## ✍️ Tu reto: aplicar acoplamiento entre servicios y sistemas
...
- Confundir el nombre del tema con una explicación de cómo acoplamiento entre servicios y sistemas afecta el sistema.
...
La fuente de este video plantea: Este video explica dos conceptos centrales en arquitectura: acoplamiento y cohesión. El acoplamiento mide cuán dependientes son componentes entre sí; la cohesión mide qué tan relacionadas están las responsabilidades dentro de un mismo módulo o servicio. La mejor arquitectura busca baja dependencia entre partes y alta claridad dentro de cada parte.
...
1. **Cuando te preguntes: ¿Qué tan acoplado está mi sistema en este momento?** Mi respuesta de partida sería: relaciona esta pregunta con la idea de que El acoplamiento alto genera fragilidad. Después busca una evidencia en el caso, no una opinión.
2. **Cuando te preguntes: ¿Qué modulos tienen demasiadas responsabilidades mezcladas?** Mi respuesta de partida sería: relaciona esta pregunta con la idea de que La cohesión alta mejora claridad y mantenibilidad. Después busca una evidencia en el caso, no una opinión.
3. **Cuando te preguntes: ¿Qué cambio actual rompe más de lo necesario por dependencias internas?** Mi respuesta de partida sería: relaciona esta pregunta con la idea de que La arquitectura debe disminuir dependencias innecesarias. Después busca una evidencia en el caso, no una opinión.
...
La alternativa A reduce el costo inicial y conserva simplicidad, pero puede dejar expuesto el riesgo principal de acoplamiento entre servicios y sistemas. La alternativa B ofrece una protección más explícita, pero agrega trabajo, dependencias o complejidad operativa. No conviene elegir B solo porque suena más moderna; debe responder a la evidencia del caso.

### ✅ Paso 5. Tomar una decisión para el MVP
Para el MVP, recomiendo elegir la alternativa que proteja primero esta idea de la fuente: El acoplamiento alto genera fragilidad.. Declara qué complejidad estás aceptando y qué señal te obligaría a cambiar la decisión.
