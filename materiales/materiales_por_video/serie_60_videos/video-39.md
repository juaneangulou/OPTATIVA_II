...
Al finalizar esta clase, quiero que puedas proteger las reglas del negocio dentro de un modelo claro y comprobable usando el caso de la plataforma logística. No quiero que memorices una definición para repetirla: quiero que aprendas a reconocer el problema, argumentar una decisión y anticipar sus consecuencias.
...
Antes de entrar en domain driven design para arquitectura limpia, deténgase en el problema que lo hace necesario. En arquitectura no aprendemos una palabra para repetirla en un diagrama; aprendemos a reconocer una situación, analizar alternativas y tomar una decisión defendible.
...
Domain Driven Design para arquitectura limpia se entiende mejor cuando lo conectamos con esta secuencia: contexto, alternativas, decisión, consecuencias y evidencia. Si falta uno de esos pasos, la propuesta queda incompleta.

## ❓ Preguntas del profesor durante la explicación
- ¿Qué parte del problema pertenece realmente a domain driven design para arquitectura limpia?
...
## 🏗️ Ejemplo resuelto
Voy a resolver una situación contigo. En la plataforma logística, una orden no puede marcarse como entregada si no existe una asignación válida y una evidencia de entrega. Observe qué parte del sistema conoce esa regla, qué información necesita y qué ocurriría si aumenta la carga, falla una dependencia o cambia la política del negocio.
...
La evidencia mínima debe incluir contexto, decisión, alternativa descartada, dos consecuencias y una forma de verificación. Guárdala en la carpeta correspondiente a Actividad 3: diseño y dominio.
...
1. **¿Qué problema resuelve esta clase?**  En este caso, una orden no puede marcarse como entregada si no existe una asignación válida y una evidencia de entrega. El problema arquitectónico consiste en organizar responsabilidades y decisiones para que ese resultado sea posible sin perder calidad, trazabilidad ni capacidad de cambio.
